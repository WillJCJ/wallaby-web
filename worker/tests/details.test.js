...
describe('handlePrivateDetails', () => {
  it('returns address and gateCode for an authenticated GET request', async () => {
    const res = handlePrivateDetails(makeRequest('GET', 'user@example.com'), baseEnv);
  ...
  it('falls back to default address and gate code when env vars are not set', async () => {
    const res = handlePrivateDetails(makeRequest('GET', 'user@example.com'), {});
    const body = await res.json();
  ...
  it('returns 401 when the auth header is absent', async () => {
    const res = handlePrivateDetails(makeRequest('GET', null), baseEnv);
  ...
  it('returns 405 for a POST request', async () => {
    const res = handlePrivateDetails(makeRequest('POST', 'user@example.com'), baseEnv);
    expect(res.status).toBe(405);
    const body = await res.json();
    expect(body.error).toBe('Method Not Allowed');
  });

  it('sets cache-control: private, no-store on the response', () => {
    const res = handlePrivateDetails(makeRequest('GET', 'user@example.com'), baseEnv);
